...
- **Unified vectors**: `Vector.h` and the old duplicate `vectors.h` are merged into a single **`vectors.h`** exposing **`vector_set`** plus **`positions_to_intervals`**, **`intervals_to_positions`**, **`positions_to_binary`** as `inline` functions in `musicpp` (one implementation; no drift between class and free helpers).
...
- **Correctness preserved**: Euclidean rhythm still uses the **pattern length** as modulus (`binary_vector::euclidean` returns `mod = static_cast<int>(result.size())`), and **positions → intervals** keeps the **cyclic closure** interval (the old loop read one past the end; that is now explicit and safe).
...
| `BinaryVector`                                     | `musicpp::binary_vector`                 |
...
| `rotoTranslate`                                    | `roto_translate`                         |
| `positionsToIntervals` (free)                      | `positions_to_intervals`                 |
| `DivisionResult` / `euclideanDivision`             | `division_result` / `euclidean_division` |


`vector_set` methods follow the same idea: **`positions()`**, **`intervals()`**, **`binary()`**, **`from_positions`** / **`from_intervals`** / **`from_binary`**, **`roto_translate_positions`**, **`update_from_positions`**, etc.

## File layout (`packages/cpp-sdk/src`)

Core: `utility.h`, `math_util.h`, `position_vector.h`, `interval_vector.h`, `binary_vector.h`, `vectors.h`.
