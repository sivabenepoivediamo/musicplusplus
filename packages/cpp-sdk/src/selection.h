...
inline position_vector select(const position_vector& source,
    ...
    const position_vector rotatedCriterion = (criterion_rotation != 0)
        ? actualCriterion.roto_translate(criterion_rotation, voices)
        : actualCriterion;

    const int outLength = (voices > 0) ? voices : static_cast<int>(rotatedCriterion.size());
    std::vector<int> result(static_cast<size_t>(outLength));

    for (int k = 0; k < outLength; ++k) {
        result[static_cast<size_t>(k)] = source[rotatedCriterion[k]];
    }
...
inline position_vector select(const position_vector& source,
    ...
    actualCriterion.set_mod(static_cast<int>(source.size()));
    const interval_vector rotatedCriterion = (criterion_rotation != 0)
        ? actualCriterion.rotate(criterion_rotation, voices)
        : actualCriterion;

    const int outLength = (voices > 0) ? voices : static_cast<int>(rotatedCriterion.size());
    std::vector<int> result(static_cast<size_t>(outLength));

    int cumulativePosition = rotatedCriterion.offset();
...
inline interval_vector select(const interval_vector& source,
    ...
    const int criterionOffset = rotatedCriterion.offset();
    ...
    for (int k = 0; k < outLength; ++k) {
        int sum = 0;
        const int spanLength = rotatedCriterion[k];
