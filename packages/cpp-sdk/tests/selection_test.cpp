...
TEST_CASE("selection_position_examples", "[selection]") {
    ...
    position_vector triad_degrees({0, 2, 4}, 7);
    ...
    position_vector extended_degrees({0, 3, 7}, 12);
...
TEST_CASE("selection_interval_examples", "[selection]") {
    ...
    interval_vector thirds({2, 2, 3}, 0, 12);
    ...
    interval_vector grouping({2}, 1, 12);
    ...
    TEST_INPUT("thirds", interval_vector({2, 2, 3}, 0, 12));
...
TEST_CASE("selection_position_from_interval_examples", "[selection]") {
    interval_vector major_scale_intervals({2, 2, 1, 2, 2, 2, 1}, 0, 12);
    position_vector positions({0, 2, 5}, 12);
    ...
    interval_vector offset_source({2, 2, 1, 2, 2, 2, 1}, 5, 12);
    ...
    position_vector single_pos({3}, 12);
...
TEST_CASE("selection_musical_and_edge_examples", "[selection]") {
    ...
    TEST_INPUT("scale", c_major_scale);
    TEST_INPUT("harmony_by_thirds", interval_vector({2}, 0, 12));
    TEST_INPUT("triad_pattern", triad_pattern);
    TEST_OUTPUT("harmony_offset_1", select(c_major_scale, interval_vector({2}, 1, 12), 0, 4));
    TEST_OUTPUT("harmony_offset_4", select(c_major_scale, interval_vector({2}, 4, 12), 0, 4));
    TEST_OUTPUT("harmony_offset_0", select(c_major_scale, interval_vector({2}, 0, 12), 0, 4));
...
TEST_CASE("selection_degenerate_examples", "[selection]") {
    position_vector c_major_scale({0, 2, 4, 5, 7, 9, 11}, 12);
    position_vector repeated_degrees({0, 0, 2}, 7);
    position_vector oversized_degrees({0, 2, 9}, 7);
    position_vector triad_degrees({0, 2, 4}, 7);
    position_vector empty_positions({}, 12);
    position_vector single_elem({5}, 12);
    position_vector repeated_single({0, 0, 0}, 1);
    interval_vector empty_interval({}, 0, 12);
    interval_vector major_scale_intervals({2, 2, 1, 2, 2, 2, 1}, 0, 12);
