...
TEST_CASE("measure_examples", "[analysis]") {
    ...
    auto raw_positions = c_major.getData();
    ...
    ASSERT_NEAR(calculateRegressionEvenness(raw_positions, c_major.getRange()), 2.28571, 1e-5);
    ...
    ASSERT_TRUE(!isEuclidean(c_major, c_major.getRange()));
    ...
    ASSERT_EQ(computeTransitionComplexity(c_major, c_major.getRange()), 10);
    ASSERT_TRUE(!isPalindrome(c_major));
    ASSERT_TRUE(!isChiral(c_major));
    ASSERT_TRUE(!isBalanced(c_major));
    ASSERT_EQ(isGenerated(raw_positions, c_major.getRange()), std::make_pair(false, -1));
    ...
    TEST_OUTPUT("regression_evenness", calculateRegressionEvenness(raw_positions, c_major.getRange()));
    ...
    TEST_OUTPUT("transition_complexity", computeTransitionComplexity(c_major, c_major.getRange()));
...
TEST_CASE("measure_false_cases", "[analysis]") {
    ...
    auto raw_positions = asymmetrical.getData();
    ...
    ASSERT_EQ(isGenerated(raw_positions, asymmetrical.getRange()), std::make_pair(false, -1));
    ASSERT_TRUE(!isEuclidean(asymmetrical, asymmetrical.getRange()));
    ASSERT_TRUE(!isPalindrome(asymmetrical));
    ASSERT_TRUE(isChiral(asymmetrical));
    ASSERT_EQ(computeTransitionComplexity(asymmetrical, asymmetrical.getRange()), 7);
    ...
    TEST_OUTPUT("generated", isGenerated(raw_positions, asymmetrical.getRange()));
    TEST_OUTPUT("euclidean", isEuclidean(asymmetrical, asymmetrical.getRange()));
    TEST_OUTPUT("palindrome", isPalindrome(asymmetrical));
    TEST_OUTPUT("chiral", isChiral(asymmetrical));
    TEST_OUTPUT("transition_complexity", computeTransitionComplexity(asymmetrical, asymmetrical.getRange()));
}
